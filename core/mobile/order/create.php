...
class Create_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function main() 
	{
		...
		if (!($packageid)) 
		{
			...
			else 
			{
				...
				$sql = 'SELECT id as goodsid,type,title,weight,issendfree,isnodiscount,ispresell,presellprice,' . ' thumb,marketprice,liveprice,islive,storeids,nosearch,isverify,deduct,hasoption,preselltimeend,presellsendstatrttime,presellsendtime,presellsendtype,' . ' manydeduct,`virtual`,maxbuy,usermaxbuy,discounts,total as stock,deduct2,showlevels,' . ' ednum,edmoney,edareas,edareas_code,unite_total,' . ' diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice,cates,minbuy, ' . ' isdiscount,isdiscount_time,isdiscount_discounts,isfullback, ' . ' virtualsend,invoice,needfollow,followtip,followurl,merchid,omerchid,checked,merchsale, ' . ' buyagain,buyagain_islong,buyagain_condition, buyagain_sale' . $threensql . $ishotelsql . ' FROM ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
				$data = pdo_fetch($sql, array(':uniacid' => $uniacid, ':id' => $id));
			...
			$nosearch = $data['nosearch'];
			if($isverify && $canverify){
				$isqr = intval($_GPC['qr']);
				if($nosearch){
					$canexpress = false;
					if(!$canverify){
					...
					if($isqr){
						...
						// 核销和时段
						$now = time();
						$time9am = strtotime(date('Y-m-d')) + 9*3600;
						$time9pm = strtotime(date('Y-m-d')) + 21*3600;
						if($now < $time9am || $now > $time9pm){
							$default_tab = false;
							$warning = 2; 
						} else {
							$before6hour = $now - 6*3600;
							if($before6hour < $time9am){
								$before6hour = $time9am;
							}
							$verifyorder = pdo_fetch('select id from '.tablename('ewei_shop_order').' where uniacid=' . $uniacid . " and openid='" . $openid . "' and verifytime>" . $before6hour . ' LIMIT 1');
		...
		if($canexpress){	
			if(!empty($exchangeOrder)){
				$postprice = $exchangepostage;				
			} else if ($taskgoodsprice > 0) {
				$postprice = $taskgoodsprice;
			} else if($dispatch_price > 0){
				$postprice = $dispatch_price;
			}
			
			if($postprice == 0 && $warning == 1){ // 不需要邮费,选快递不提醒
				$warning = 0;
			} else {
				if($default_tab){
					$realprice -= $postprice;
		...
		$createInfo['warning'] = $warning;
		$createInfo['postprice'] = $postprice > 0 ? 1: 0;
		//$createInfo['iscarry'] = $postprice > 0 && !$default_tab ? 0 : 1;
		include $this->template();
	}
	...
	public function caculate() 
	{
		...
		$return_array = array();
		$return_array['dd'] = $dispatch_array;
	...
	public function submit() 
	{
		...
		if (!($isvirtual) && !($isonlyverifygoods) && ($dispatchtype == 0) && !($isonlyverifygoods)) 
		{
			if (empty($addressid)) 
			{
				show_json(0, '请选择地址');
		...
		if ($multiple_order == 0) 
		{
			...
			$order['isparent'] = 0;
			$order['transid'] = '';
			$order['isverify'] = $order['dispatchtype']; // dispatchtype == iscarry == isverify
