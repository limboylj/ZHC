...
class Detail_EweiShopV2Page extends MobilePage 
{
	public function main() 
	{
		...
		$isqr = intval($_GPC['qr']);
		if($goods['isverify'] == 2 && $goods['nosearch'] && (!$isqr || empty($stores))){
			$goods['canbuy'] = false;
		}
		if (com('coupon')) 
		{
		...
		$commission_money = null;
		if(empty($goods['nocommission'])){
			if(!empty($goods['hascommission'])){
				if($goods['commission1_rate'] > 0 || $goods['commission2_rate'] > 0){
					$commission_money = [number_format($goods['marketprice']*$goods['commission1_rate']/100,2),number_format($goods['marketprice']*$goods['commission2_rate']/100,2)];
				}else if($goods['commission1_pay'] > 0 || $goods['commission2_pay'] > 0){
					$commission_money = [number_format($goods['commission1_pay'],2),number_format($goods['commission2_pay'],2)];
				}
			} else {
				$commission_money = [number_format($goods['marketprice']*$_S['commission']['commission1']/100,2),number_format($goods['marketprice']*$_S['commission']['commission2']/100,2)];
			}
		}
