...
class Uploader_EweiShopV2Page extends MobilePage 
{
	...
	protected function upload($uploadfile) 
	{
		...
		$thumbwidth = intval($_GPC['width']);
		if($thumbwidth > 0){				
			$result['filename'] = file_image_thumb(ATTACHMENT_ROOT.$file['path'],'',$thumbwidth);
			file_delete(ATTACHMENT_ROOT.$file['path']);	
		}
	...
	public function remove() 
	{
		...
		if(stripos($file,$_W['siteroot']) === 0){
			$file = str_ireplace($_W['siteroot'],'',$file);
		}
