...
{
	protected function orderData($status, $st, $ismerch = false) 
	{
		global $_W;
		global $_GPC;
		...
		if($ismerch){
			...
			if(!empty($search_merchid)){
				$condition .= ' and o.merchid=' . $search_merchid;
			} else {
		...
		if (!(empty($list))) 
		{
			...
			foreach ($list as &$value ) 
			{
				if ($is_merchname == 1) 
				{
					$value['merchname'] = (($merch_user[$value[$merchid_column]]['merchname'] ? $merch_user[$value[$merchid_column]]['merchname'] : ''));
				}
				$s = $value['status'];
				$pt = $value['paytype'];
				$value['statusvalue'] = $s;
	...
	public function _main() 
	{
		global $_W;
		global $_GPC;
		$orderData = $this->orderData('', 'main',true);
	}
	public function _status0() 
	{
		global $_W;
		global $_GPC;
		$orderData = $this->orderData(0, 'status0',true);
	}
	public function _status1() 
	{
		global $_W;
		global $_GPC;
		$orderData = $this->orderData(1, 'status1',true);
	}
	public function _status2() 
	{
		global $_W;
		global $_GPC;
		$orderData = $this->orderData(2, 'status2',true);
	}
	public function _status3() 
	{
		global $_W;
		global $_GPC;
		$orderData = $this->orderData(3, 'status3',true);
	}
	public function _status4() 
	{
		global $_W;
		global $_GPC;
		$orderData = $this->orderData(4, 'status4',true);
	}
	public function _status5() 
	{
		global $_W;
		global $_GPC;
		$orderData = $this->orderData(5, 'status5',true);
	}
	public function _status_1() 
	{
		global $_W;
		global $_GPC;
		$orderData = $this->orderData(-1, 'status_1',true);
	}
