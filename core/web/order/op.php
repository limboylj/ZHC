...
class Op_EweiShopV2Page extends WebPage 
{
	...
	public function pay($a = array(), $b = array()) 
	{
		...
		else 
		{
			...
			//cannotrefund 付款后开始全返
			$goods = pdo_fetch('select id,goodsid,total,realprice from ' . tablename('ewei_shop_order_goods') . ' where  orderid=:orderid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			$g = pdo_fetch('select id,credit,sales,salesreal,cannotrefund from ' . tablename('ewei_shop_goods') . ' where  id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $goods['goodsid']));
			//if($g['cannotrefund']==1){
	...
	public function finish() 
	{
		...
		//m('order')->setStocksAndCredits($item['id'], 3);//为什么用3
		//m('order')->setStocksAndCredits($item['id'], 1);
