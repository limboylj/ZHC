...
class Witchain_EweiShopV2Page extends WebPage 
{
	protected function main($type = 0) 
	{
		...
		$sql = 'select log.id,log.openid,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,log.money,log.createtime,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype,m.nickname,m.id as mid,m.avatar,m.level,m.groupid,m.realname,m.mobile,g.groupname,l.levelname from ' . tablename('ewei_shop_member_withdrawdividend_log') . ' log ' . ' left join ' . tablename('ewei_shop_member') . ' m on m.openid = log.openid ' . ' left join ' . tablename('ewei_shop_member_group') . ' g on g.id = m.groupid ' . ' left join ' . tablename('ewei_shop_member_level') . ' l on l.id = m.level ' . ' where 1 ' . $condition . ' ' . $condition1 . ' GROUP BY log.id ORDER BY log.createtime DESC ';
		...
		if ($_GPC['export'] == 1) 
		{
			if ($_GPC['type'] == 1) 
			{
				plog('finance.witchain.withdraw.export', '导出提现记录');
			}
			else 
			{
				plog('finance.witchain.recharge.export', '导出充值记录');
			}
			foreach ($list as &$row ) 
			{
		...
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member_withdrawdividend_log') . ' log ' . ' where 1 ' . $condition . ' ' . $member_sql, $params);
	...
	public function wechat() 
	{
		...
		{
			$result = m('finance')->payRedPack($log['openid'], $realmoney * 100, $log['logno'], $log, $set['name'] . '红利提现', $data['paytype']);
		...
		{
			$result = m('finance')->pay($log['openid'], 1, $realmoney * 100, $log['logno'], $set['name'] . '红利提现');
		}
		if (is_error($result)) 
		{
		...
		plog('finance.witchain.wechat', '红利提现 ID: ' . $log['id'] . ' 方式: 微信 提现金额: ' . $log['money'] . ' ,到账金额: ' . $realmoney . ' ,手续费金额 : ' . $log['deductionmoney'] . '<br/>会员信息:  ID: ' . $member['id'] . ' / ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		show_json(1);
	}
	...
	public function manual() 
	{
		...
		pdo_update('ewei_shop_member_withdrawdividend_log', array('status' => 1), array('id' => $id, 'uniacid' => $_W['uniacid']));
		m('notice')->sendDividendLogMessage($log['id']);
		plog('finance.witchain.manual', '红利提现 方式: 手动 ID: ' . $log['id'] . ' <br/>会员信息: ID: ' . $member['id'] . ' / ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		show_json(1);
	}
	public function refuse() 
	{
		...
		if (0 < $log['money']) 
		{
			m('member')->setCredit8($log['openid'], 'increased', $log['money'], array(0, $set['name'] . '红利提现退回'));
		}
		m('notice')->sendDividendLogMessage($log['id']);//红利拒绝提现通知
		plog('finance.witchain.refuse', '拒绝红利度提现 ID: ' . $log['id'] . ' 金额: ' . $log['money'] . ' <br/>会员信息:  ID: ' . $member['id'] . ' / ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		show_json(1);
	}
