...
class Dividendlog_EweiShopV2Page extends WebPage 
{
	protected function main($type = 0) 
	{
		...
		$sql = 'select log.id,m.id as mid, m.realname,m.avatar,m.weixin,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,m.nickname,m.mobile,g.groupname,log.money,log.createtime,l.levelname,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype,log.creditducaion from ' . tablename('ewei_shop_member_withdrawdividend_log') . ' log ' . ' left join ' . tablename('ewei_shop_member') . ' + on m.openid=log.openid' . ' left join ' . tablename('ewei_shop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('ewei_shop_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC ';
		...
		if ($_GPC['export'] == 1) 
		{
			...
			m('excel')->export($list, array('title' => ((empty($type) ? '会员充值数据-' : '会员红利提现记录')) . date('Y-m-d-H-i', time()), 'columns' => $columns));
		}
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member_withdrawdividend_log') . ' log ' . ' left join ' . tablename('ewei_shop_member') . ' m on m.openid=log.openid and m.uniacid= log.uniacid' . ' left join ' . tablename('ewei_shop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('ewei_shop_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ', $params);
	...
	public function wechat() 
	{
		...
		$log = pdo_fetch('select * from ' . tablename('ewei_shop_member_withdrawdividend_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (empty($log)) 
		{
			show_json(0, '未找到记录!');
		}
		...
		{
			$result = m('finance')->payRedPack($log['openid'], $realmoney * 100, $log['logno'], $log, $set['name'] . $witset['dividendtext'].'提现', $data['paytype']);
			pdo_update('ewei_shop_member_withdrawdividend_log', array('sendmoney' => $result['sendmoney'], 'senddata' => json_encode($result['senddata'])), array('id' => $log['id']));
		...
		else 
		{
			...
			$result = m('finance')->pay($log['openid'], 1, $realmoney * 100, $log['logno'], $set['name'] . $witset['dividendtext'].'提现');
		}
		if (is_error($result)) 
		{
			show_json(0, array('message' => $result['message']));
		}
		$flag=pdo_update('ewei_shop_member_withdrawdividend_log', array('status' => 1), array('id' => $id, 'uniacid' => $_W['uniacid']));
		if($flag!==false){
			m('member')->setCredit($log['openid'], 'credit1', $log['creditducaion'], array(0, $set['name'] . $witset['dividendtext'].'转换积分'));
		}


		m('notice')->sendDividendLogMessage($log['id']);
		$member = m('member')->getMember($log['openid']);
		plog('finance.coinlog.wechat', $witset['dividendtext'].'提现 ID: ' . $log['id'] . ' 方式: 微信 '.$witset['dividendtext'].'提现: ' . $log['money'] . ' ,到账金额: ' . $realmoney . ' ,手续费金额 : ' . $log['deductionmoney'] . ',积分转换 : ' . $log['creditducaion'] . '<br/>会员信息:  ID: ' . $member['id'] . ' / ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		show_json(1);
	}
	...
	public function manual() 
	{
		...
		m('notice')->sendDividendLogMessage($log['id']);
		plog('finance.coinlog.manual', $tradeset['cointext'].'提现 方式: 手动 ID: ' . $log['id'] . ' <br/>会员信息: ID: ' . $member['id'] . ' / ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		show_json(1);
	}
	public function refuse() 
	{
		...
		pdo_update('ewei_shop_member_withdrawdividend_log', array('status' => -1), array('id' => $id, 'uniacid' => $_W['uniacid']));
		if (0 < $log['money']) 
		{
			m('member')->setCredit8($log['openid'], 'credit8', $log['money'], array(0, $set['name'] . $witset['dividendtext'].'提现退回'));
			
		}
		m('notice')->sendDividendLogMessage($log['id']);
		plog('finance.coinlog.refuse', '拒绝'.$witset['dividendtext'].'提现 ID: ' . $log['id'] . ' 金额: ' . $log['money'] . ' <br/>会员信息:  ID: ' . $member['id'] . ' / ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		show_json(1);
	}
