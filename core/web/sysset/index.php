...
class Index_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		/*else if (cv('sysset.dividend')) 
		{
			header('location: ' . webUrl('sysset/dividend'));
		}*/
		else if (cv('sysset.witchain')) 
		{
			header('location: ' . webUrl('sysset/witchain'));
		}
		else 
		{
	...
	//智汇链基础设置
	public function fullbackmoney(){
		global $_W;
		global $_GPC;
		
		if($_W['ispost']){
			ca('sysset.fullbackmoney.edit');
			$data = ((is_array($_GPC['data']) ? $_GPC['data'] : array()));
			
			
			if (!(empty($data['fullbackwithdrawcharge']))) 
			{
				$data['fullbackwithdrawcharge'] = trim($data['fullbackwithdrawcharge']);
				$data['fullbackwithdrawcharge'] = floatval(trim($data['fullbackwithdrawcharge'], '%'));
			}
			
			$data['fullbackwithdrawbegin'] = floatval(trim($data['fullbackwithdrawbegin']));
			$data['fullbackwithdrawend'] = floatval(trim($data['fullbackwithdrawend']));
			
			$data['fullbackwithdrawcashweixin'] = intval($data['fullbackwithdrawcashweixin']);
			$data['fullbackwithdrawcashalipay'] = intval($data['fullbackwithdrawcashalipay']);
			$data['fullbackwithdrawcashcard'] = intval($data['fullbackwithdrawcashcard']);
		
			
			m('common')->updateSysset(array('fullbackmoney' => $data));
			plog('sysset.fullbackmoney.edit', '修改系统设置-补贴设置');
			show_json(1);
		}
		$data=m('common')->getSysset("fullbackmoney");
		include $this->template();
	}
	// 智汇链
	public function witchain(){
		global $_W;
		global $_GPC;

		if ($_W['ispost']) 
		{
			ca('sysset.witchain.edit');
			$data = ((is_array($_GPC['data']) ? $_GPC['data'] : array()));
			
			if (!(empty($data['dividendwithdrawcharge']))) 
			{
				$data['dividendwithdrawcharge'] = trim($data['dividendwithdrawcharge']);
				$data['dividendwithdrawcharge'] = floatval(trim($data['dividendwithdrawcharge'], '%'));
			}
			$data['minimumdividendcharge'] = floatval(trim($data['minimumdividendcharge']));
			$data['dividendwithdrawbegin'] = floatval(trim($data['dividendwithdrawbegin']));
			$data['dividendwithdrawend'] = floatval(trim($data['dividendwithdrawend']));
			
			$data['dividendwithdrawcashweixin'] = intval($data['dividendwithdrawcashweixin']);
			$data['dividendwithdrawcashalipay'] = intval($data['dividendwithdrawcashalipay']);
			$data['dividendwithdrawcashcard'] = intval($data['dividendwithdrawcashcard']);

			$dividendFrom=$_GPC['dividendfrom'];
			$dividendTo=$_GPC['dividendto'];
			$dividendRate=$_GPC['dividendrate'];
			
			$arr=array();
			if(!empty($dividendFrom) && !empty($dividendTo) && !empty($dividendRate)){
				foreach($dividendFrom as $kk=>$vv){
					$arr[$kk]['dividendfrom']=$vv;
					$arr[$kk]['dividendto']=$dividendTo[$kk]?$dividendTo[$kk]:0;
					$arr[$kk]['dividendrate']=$dividendRate[$kk]?$dividendRate[$kk]:0;
				}
			}

			$arrs['data']=iserializer($arr);
			$data['dividenddata']=$arrs;
			//pv
			$data['dividenpv']=floatval($data['dividenpv']);
			//
			m('common')->updateSysset(array('witchain' => $data));
			plog('sysset.witchain.edit', '修改系统设置-智汇链设置');
			show_json(1);
		}
		
		$data = m('common')->getSysset('witchain');
		$item=$data['dividenddata'];
		$item['data']=iunserializer($item['data']);
		$item['template']=1;
		include $this->template();
	}
	public function witchaintpl(){
		include $this->template("sysset/witchain/tpl");
	}
}
?>