...
class Index_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		$result=pdo_fetchall("select fb.goodsid,g.cannotrefund,g.id as ttid from ".tablename('ewei_shop_fullback_goods')." fb left join ".tablename('ewei_shop_goods')." g on g.id=fb.goodsid where fb.uniacid=".$_W['uniacid']);
		if(!empty($result)){
			foreach($result as $k=>$v){
				pdo_update("ewei_shop_goods",array('cannotrefund'=>1),array('id'=>$v['ttid']));
			}
		}
