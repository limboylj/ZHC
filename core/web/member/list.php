...
class List_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		if ($_GPC['mobileverify'] != '') 
		{
			$condition .= ' and dm.mobileverify=' . intval($_GPC['mobileverify']);
			$issearch = true;
		}
		...
		foreach ($list as &$row ) 
		{
			...
			// $downbase = pdo_fetchcolumn("select ifnull(sum(bases),0) from ".tablename('ewei_shop_member')." where uniacid=".$_W['uniacid']." and agentid=".$row['id']);
			// $row['downbase']=$downbase*$set['witchain']['downwittoup']/100;
			$row['downbase']=m('member')->getLowerBase($row['id']);
			$pv = pdo_fetchcolumn("select ifnull(sum(pv),0) from ".tablename('ewei_shop_member')." where uniacid=".$_W['uniacid']." and agentid=".$row['id']);
			$reduce_pv = pdo_fetchcolumn("select ifnull(sum(money),0) from ".tablename('ewei_shop_member_withdrawdividend_log')." where uniacid=".$_W['uniacid']." and status >= 0 and openid='".$row['openid']."'");
			$row['pv'] = $pv - $reduce_pv;
		}
		unset($row);
		if ($_GPC['export'] == '1') 
		...
		$today_join = pdo_fetchcolumn('select count(*) from' . tablename('ewei_shop_member') . ' dm ' . $join . ' where 1 ' . $condition . ' and createtime > '. strtotime(date('Y-m-d')), $params);
