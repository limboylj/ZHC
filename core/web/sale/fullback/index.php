...
class Index_EweiShopV2Page extends WebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) {
			...
			$good_data = pdo_fetch('select title,thumb,marketprice,goodssn,productsn,hasoption,witchain' . "\n" . '                            from ' . tablename('ewei_shop_goods') . ' where id = ' . $data['goodsid'] . ' and uniacid = ' . $uniacid . ' ');

			if(!empty($good_data['witchain'])){
				show_json(0,'购物直接送智慧链的商品不能进行全返');
			}
			...
			if (empty($good_data['option']) && !($good_data['hasoption'])) {
				...
				$data['day_least'] = $fullbackgoodsArray[5];
			}
			...
			if (!(empty($good_data['option']))) {
				...
				foreach ($fullbackOption as $val ) {
					...
					$optionData = array('allfullbackprice' => floatval($fullbackgoodsoption[0]), 'fullbackprice' => floatval($fullbackgoodsoption[1]), 'allfullbackratio' => floatval($fullbackgoodsoption[2]), 'fullbackratio' => floatval($fullbackgoodsoption[3]), 'day' => intval($fullbackgoodsoption[4]), 'isfullback' => 1,'day_least'=>intval($fullbackgoodsoption[5]));
			...
			//
            if($data['refund']==0){
				pdo_update('ewei_shop_goods',array('cannotrefund'=>1),array('id' => $data['goodsid']));
			}
			if (0 < $data['status']) {
	...
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			pdo_update('ewei_shop_goods', array('isfullback' => intval($_GPC['status'])), array('id' => $item['goodsid']));
	...
	public function query() 
	{
		...
		$condition = ' and deleted=0 and uniacid=:uniacid and status = 1 and merchid = 0 and type != 30 and (witchain="" or witchain is null) ';//加一个条件，如果商品购买即送智慧链，不能用于全返
