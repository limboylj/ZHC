...
{
	public function main($goodsfrom = 'sale',$ismerch=false)
	{
		global $_W;
		global $_GPC;
		...
		$sqlcondition .= ' left join ' . tablename('ewei_shop_fullback_goods') . ' fg on g.id = fg.goodsid';
		...
		$merchid_column = 'merchid';
		if($ismerch){
			$search_merchid = $_GPC['merchid'];
			if(!empty($search_merchid)){
				$condition .= ' and g.merchid=' . $search_merchid;
			} else {
				$condition .= ' and g.merchid>0 ';
			}			
		} else {
			$condition .= ' and g.merchid=0 ';
			$merchid_column = 'omerchid';
		}
		...
		if (!empty($total)) {
			$sql = 'SELECT g.*,fg.status as fstatus,fg.minallfullbackallprice,fg.fullbackprice,fg.units FROM ' . tablename('ewei_shop_goods') . 'g' . $sqlcondition . $condition . $groupcondition . " ORDER BY g.`status` DESC, g.`displayorder` DESC,\r\n                g.`id` DESC LIMIT " . (($pindex - 1) * $psize) . ',' . $psize;
			$list = pdo_fetchall($sql, $params);
			
			foreach ($list as $key => &$value) {
				$url = mobileUrl('goods/detail', array('id' => $value['id'],'qr'=>1), true);
			...
			if ($merch_plugin) {
				$merch_user = $merch_plugin->getListUser($list, 'merch_user', $merchid_column);
				if (!empty($list) && !empty($merch_user)) {
					foreach ($list as &$row) {
						$row['merchname'] = $merch_user[$row[$merchid_column]]['merchname'] ? $merch_user[$row[$merchid_column]]['merchname'] : '';
					}
				}
				
				$merch_users = $merch_plugin->getMerch();
			}
		}
		...
		if($ismerch){
			include $this->template('goods/indexmerch');
		} else {
	...
	{
		$this->main('stock',true);
	}

	public function mcycle()
	{
		$this->main('cycle',true);
	}
	...
	public function status()
	{
		...
		foreach ($items as $item) {
			...
			pdo_update('ewei_shop_goods', array($colname => intval($_GPC[$colname])), array('id' => $item['id']));
			if($colname == 'status'){
				plog('goods.edit', (('修改商品状态<br/>ID: ' . $item['id'] . '<br/>商品名称: ' . $item['title'] . '<br/>状态: ' . $_GPC['status']) == 1 ? '上架' : '下架'));
			} else if($colname == 'nosearch'){
				plog('goods.edit', (('修改商品状态<br/>ID: ' . $item['id'] . '<br/>商品名称: ' . $item['title'] . '<br/>状态: ' . $_GPC['nosearch']) == 1 ? '隐藏' : '显示'));
			} else if($colname == 'isfullback'){
				//plog('goods.edit', (('修改商品状态<br/>ID: ' . $item['id'] . '<br/>商品名称: ' . $item['title'] . '<br/>状态: ' . $_GPC['nosearch']) == 1 ? '隐藏' : '显示'));
			}
		}
	...
	public function change()
	{
		...
		if (!in_array($type, array('title', 'marketprice', 'total', 'goodssn', 'productsn', 'displayorder', 'dowpayment','costprice','credit','deduct'))) {
			show_json(0, array('message' => '参数错误'));
		}
