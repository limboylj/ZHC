...
/**
 * [getRandomNum description]
 * @param  [type] $total [description]待划分的数字
 * @param  [type] $div   [description]划分数
 * @param  [type] $area  [description]各份数间允许的最大差值
 * @return [type] $po    [description]小数点位数
 * @return [type]        [description]
 */
function getRandomNum($total,$div,$area,$po){ 
	$average = round($total / $div,$po);
	$sum = 0;
	...
	if( $sum > 0 ){
	 ...
	 //产生各份的份额
	 $random = round($min + mt_rand()/mt_getrandmax() * ($max-$min),$po); 
	 //$random =round(($min+ lcg_value().PHP_EOL),6);
	 $sum += $random;
...
/**
 * [getAverageNum description]
 * @param  [type] $div   [description]分成几份
 * @param  [type] $total [description]总数
 * @param  [type] $po    [description]小数点后位数
 * @param  [type] $area    [description]范围
 * @return [type]        [description]
 */
function getAverageNum($total,$div,$po=2,$area=0){
	if($area==0){
		$area=round($total/$div/10,$po);//步长按平均数的10分之1
		if($total<0.1){
			$area=number_format($total/$div/10,$po);
		}
	}
	$a = array_fill( 1, $div, 0 );
	$n=1;
	foreach($a as $k=>$v){
		$a[$k]=$v+$n*$area;//步长
		$n++;
	...
	$base=round(($total-array_sum($a))/$div,$po);
	$tmpSum=0;
	foreach($a as $ak=>$av){
		if($ak<count($a)){
			$a[$ak]=$av+$base;
...
/*function getAverageNum($total,$div,$area,$po=2){
	$a=range(0,$div-1,0.2);
