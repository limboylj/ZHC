...
class MobilePage extends Page 
{
	...
	public function __construct() 
	{
		...
		if($do=='mobile' && $r!='commission.addcredits' && $r!='order.create.calculate' && $r!='util.task' && strpos($r,'member.cart')===false && $r!='goods.detail' && strpos($r,'goods.detail')===false){//当执行方法不是定时任务时
			//$this->str=$this->memberCredit();
			m('member')->memberCredit();
		}
	...
	/**
	 * [followBar description]
	 * @param  boolean $diypage [description]
	 * @param  boolean $merch   [description]
				...
				if (!(empty($set['level']))) 
				{
					...
					if (!(empty($member)) && ($member['status'] == 1) && ($member['isagent'] == 1)) 
					{
						if (empty($set['closemyshop'])) 
						{
							...
							$_W['shopshare'] = array('title' => $myshop['name'], 'imgUrl' => tomedia($myshop['logo']), 'desc' => $myshop['desc'], 'link' => mobileUrl($_W['routes'], array('mid' => $member['id']), true));
						}
						else 
						{
