...
class Op_EweiShopV2Page extends MerchWebPage 
{
	...
	protected function opData() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_order') . ' WHERE id = :id and uniacid=:uniacid and (merchid = :merchid or omerchid = :merchid)', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		if (empty($item)) 
		{
			if ($_W['isajax']) 
			{
	...
	public function finish() 
	{
		...
		if(!empty($item['omerchid'])){
			pdo_update('ewei_shop_order', array('status' => 3, 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'omerchid' => $item['omerchid']));
		} else {
	...
	public function fetch() 
	{
		...
		if(!empty($item['omerchid'])){
			pdo_update('ewei_shop_order', $d, array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'omerchid' => $item['omerchid']));
		} else {
	...
	public function send() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$time = time();
			if(!empty($item['omerchid'])){
				pdo_update('ewei_shop_order', array('status' => 2, 'express' => trim($_GPC['express']), 'expresscom' => trim($_GPC['expresscom']), 'expresssn' => trim($_GPC['expresssn']), 'sendtime' => $time), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'omerchid' => $item['omerchid']));
			} else {
	...
	public function changeexpress() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($expresssn))) 
			{
				...
				$result = pdo_update('ewei_shop_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
				if(!$result){
					pdo_update('ewei_shop_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid'], 'omerchid' => $_W['merchid']));
				}
