...
class MerchModel extends PluginModel 
{
	...
	public function getListUser($list, $return = 'all', $idstr = 'merchid') 
	{
		global $_W;
		if (!(is_array($list))) 
		{
	...
	public function getMerchOrderTotalPrice($merchid) 
	{
		...
		$data['deductprice']=$list['deductprice']?floatval($list['deductprice']):0;
		...
		foreach ($status1 as $k => $v ) 
		{
			...
			//zhq
				...
				$status1orderids =iunserializer($v['orderids']);
			}
		}
		...
		{
			$status1order = pdo_fetchall('SELECT id,deductprice FROM ' . tablename('ewei_shop_order') . ' WHERE id in(' . join(',', $status1orderids) . ') AND uniacid=' . $_W['uniacid']);
			...
			if (!(empty($status1order))) 
			{
				foreach ($status1order as $k => $v ) 
				{
					...
					//
					$deductprice1+=$v['deductprice'];
				}
			}
			$data['commission1'] = round($commission1, 2);
			$data['deductprice1'] = round($deductprice1, 2);
		}
		...
		foreach ($status2 as $k => $v ) 
		{
			...
			//zhq
				...
				$status2orderids= iunserializer($v['orderids']);
			}
		}
		...
		{
			$status2order = pdo_fetchall('SELECT id,deductprice FROM ' . tablename('ewei_shop_order') . ' WHERE id in(' . join(',', $status2orderids) . ') AND uniacid=' . $_W['uniacid']);
			$commission2 = 0;
			$deductprice2=0;
			if (!(empty($status2order))) 
			{
				foreach ($status2order as $k => $v ) 
				{
					...
					$deductprice2+=$v['deductprice'];
				}
			}
			$data['commission2'] = round($commission2, 2);
			$data['deductprice2'] = round($deductprice2, 2);
		}
		...
		foreach ($status3 as $k => $v ) 
		{
			...
			//zhq
				...
				$status3orderids=iunserializer($v['orderids']);
			}
		}
		...
		{
			$status3order = pdo_fetchall('SELECT id,deductprice FROM ' . tablename('ewei_shop_order') . ' WHERE id in(' . join(',', $status3orderids) . ') AND uniacid=' . $_W['uniacid']);
			$commission3 = 0;
			$deductprice3=0;
			if (!(empty($status3order))) 
			{
				foreach ($status3order as $k => $v ) 
				{
					...
					$deductprice3+=$v['deductprice'];
				}
			}
			$data['commission3'] = round($commission3, 2);
			$data['deductprice3'] = round($deductprice3, 2);
		}
		return $data;
	}
	public function getMerchPrice($merchid, $flag = 0) 
	{
		...
		if (!(empty($merch_data['deduct_credit1']))) 
		{
			$deduct_credit1 = 1;
		}
		else 
		...
		if($deduct_credit1){
			$list['realprice']-=$list['deductprice'];
		}
		$list['realprice']=$list['realprice']>0?floatval($list['realprice']):0;
