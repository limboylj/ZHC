...
class Apply_EweiShopV2Page extends CommissionMobileLoginPage
{
	public function main()
	{
		...
		if($towit){
			if (empty($backset['isfullbacktowit'])) 
			{
		...
		if ($_W['ispost']) {
			if(empty($_GPC['towit'])){
				...
				foreach ($orderids as $o) {
					pdo_update('ewei_shop_order_goods', array('status' . $o['level'] => 4, 'applytime' . $o['level'] => $time), array('orderid' => $o['orderid'], 'uniacid' => $_W['uniacid']));
				}
				...
				if ($money <= 0) 
				{
					show_json(0, $this->set['texts']['commission_ok'].'为0不能转换!');
				}

				//zhq
				if($witset['maxbases']>0 && ($member['bases']+$money)>$witset['maxbases']){

					show_json(0,'最大'.$witset['wittext'].'不能超过'.$witset['maxbases']);
				}
				
				//
				if ($money < floatval($backset['fullbacktowitless'])) 
				{
					show_json(0, $this->set['texts']['commission_ok'].'不足'.$backset['fullbacktowitless'].$this->set['texts']['yuan'].'不能转换!');
				}
				foreach ($orderids as $o) {
				...
				$apply = [
					...
					'openid' => $_W['openid'],
					'logno' => $logno,
					'fromtype' => 2, // 0补贴提现计入， 1 补贴直接转换 ，2 佣金
					'title' => $this->set['commission'].'转换计入到pv',
					'createtime' => time(),
					'money' => $money,
					'withdrawtopv' => 0
				];
