...
if (!(class_exists('CommissionModel'))) 
{
	class CommissionModel extends PluginModel 
	{
		public function getSet($uniacid = 0) 
		{
			...
			$set['texts'] = array('agent' => (empty($set['texts']['agent']) ? '分销商' : $set['texts']['agent']), 'shop' => (empty($set['texts']['shop']) ? '小店' : $set['texts']['shop']), 'myshop' => (empty($set['texts']['myshop']) ? '我的小店' : $set['texts']['myshop']), 'center' => (empty($set['texts']['center']) ? '分销中心' : $set['texts']['center']), 'become' => (empty($set['texts']['become']) ? '成为分销商' : $set['texts']['become']), 'withdraw' => (empty($set['texts']['withdraw']) ? '提现' : $set['texts']['withdraw']), 'commission' => (empty($set['texts']['commission']) ? '佣金' : $set['texts']['commission']), 'commission1' => (empty($set['texts']['commission1']) ? '分销佣金' : $set['texts']['commission1']), 'commission_total' => (empty($set['texts']['commission_total']) ? '累计佣金' : $set['texts']['commission_total']), 'commission_ok' => (empty($set['texts']['commission_ok']) ? '可提现佣金' : $set['texts']['commission_ok']), 'commission_apply' => (empty($set['texts']['commission_apply']) ? '已申请佣金' : $set['texts']['commission_apply']), 'commission_check' => (empty($set['texts']['commission_check']) ? '待打款佣金' : $set['texts']['commission_check']), 'commission_lock' => (empty($set['texts']['commission_lock']) ? '未结算佣金' : $set['texts']['commission_lock']), 'commission_detail' => (empty($set['texts']['commission_detail']) ? '提现明细' : (($set['texts']['commission_detail'] == '佣金明细' ? '提现明细' : $set['texts']['commission_detail']))), 'commission_pay' => (empty($set['texts']['commission_pay']) ? '成功提现佣金' : $set['texts']['commission_pay']), 'commission_wait' => (empty($set['texts']['commission_wait']) ? '待收货佣金' : $set['texts']['commission_wait']), 'commission_fail' => (empty($set['texts']['commission_fail']) ? '无效佣金' : $set['texts']['commission_fail']), 'commission_trans' => (empty($set['texts']['commission_trans']) ? '成功转换佣金' : $set['texts']['commission_trans']), 'commission_charge' => (empty($set['texts']['commission_charge']) ? '扣除提现手续费' : $set['texts']['commission_charge']), 'order' => (empty($set['texts']['order']) ? '分销订单' : $set['texts']['order']), 'c1' => (empty($set['texts']['c1']) ? '一级' : $set['texts']['c1']), 'c2' => (empty($set['texts']['c2']) ? '二级' : $set['texts']['c2']), 'c3' => (empty($set['texts']['c3']) ? '三级' : $set['texts']['c3']), 'mydown' => (empty($set['texts']['mydown']) ? '我的下线' : $set['texts']['mydown']), 'down' => (empty($set['texts']['down']) ? '下线' : $set['texts']['down']), 'up' => (empty($set['texts']['up']) ? '推荐人' : $set['texts']['up']), 'yuan' => (empty($set['texts']['yuan']) ? '元' : $set['texts']['yuan']), 'icode' => (empty($set['texts']['icode']) ? '邀请码' : $set['texts']['icode']));
			return $set;
		}
		...
		public function getInfo($openid, $options = NULL) 
		{
			...
			if (1 <= $level) 
			{
				...
				if (in_array('trans', $options)) 
				{
					$level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status=3 and og.status1=4 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
					foreach ($level1_commissions2 as $c ) 
					{
						$commissions = iunserializer($c['commissions']);
						$commission = iunserializer($c['commission1']);
						if (empty($commissions)) 
						{
							$commission_trans += ((isset($commission['level' . $agentLevel['id']]) ? $commission['level' . $agentLevel['id']] : $commission['default']));
						}
						else 
						{
							$commission_trans += ((isset($commissions['level1']) ? $commissions['level1'] : 0));
						}
					}
				}
			...
			if (2 <= $level) 
			{
				if (0 < $level1) 
				{
					...
					if (in_array('trans', $options)) 
					{
						$level2_commissions3 = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status=3 and og.status2=4 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
						foreach ($level2_commissions3 as $c ) 
						{
							$commissions = iunserializer($c['commissions']);
							$commission = iunserializer($c['commission2']);
							...
							{
								$commission_trans += ((isset($commissions['level2']) ? $commissions['level2'] : 0));
							}
						}
					}
			...
			if (3 <= $level) 
			{
				if (0 < $level2) 
				{
					...
					if (in_array('trans', $options)) 
					{
						$level3_commissions3 = pdo_fetchall('select og.commission3,og.commissions  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status=3 and og.status3=4 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
						foreach ($level3_commissions3 as $c ) 
						{
							$commissions = iunserializer($c['commissions']);
							$commission = iunserializer($c['commission3']);
							...
							{
								$commission_trans += ((isset($commissions['level3']) ? $commissions['level3'] : 0));
							}
						}
					}
			...
			$member['commission_trans'] = round($commission_trans, 2);
		...
		public function checkOrderFinish($orderid = '') 
		{
			...
			if($order['agentid']){
					...
					$fullbacklogs=pdo_fetchall("select * from ".tablename("ewei_shop_fullback_log")." where uniacid=".$_W['uniacid']." and openid='".$agentinfo['openid']."' and isfullback=0 and fullbackday < day");				
						if($fullbacklogs){
						  foreach($fullbacklogs as $key=>$fullbacklog){
							
							if(($fullbacklog['day']-$fullbacklog['fullbackday'])>1){//如果剩余全返大于1天
								$is_recommond=pdo_fetch("select id from ".tablename('ewei_shop_fullback_recommend_log')." where uniacid=".$_W['uniacid']." and logid=".$fullbacklog['id']." and status=1 and openid='".$order['openid']."'");
								
								if(empty($is_recommond)){
									$adds=pdo_fetch("select fullbackprice_add,day_least from ".tablename('ewei_shop_fullback_goods')." where uniacid=".$_W['uniacid']." and goodsid=".$fullbacklog['goodsid']);
									
									//重新计算每天返现数和天数
									$newpriceevery=$fullbacklog['priceevery']+$adds['fullbackprice_add'];
									// if($newpriceevery>=($fullbacklog['price']-$fullbacklog['fullbackedmoney'])){
									// 	$newpriceevery=$fullbacklog['price']-$fullbacklog['fullbackedmoney'];
									// }
									$leftdays=($fullbacklog['price']-$fullbacklog['fullbackedmoney'])/$newpriceevery;
									$leftdays=ceil($leftdays);
									$newday=$fullbacklog['fullbackday']+$leftdays;
									pdo_update('ewei_shop_fullback_log',array('day'=>$newday,'priceevery'=>$newpriceevery),array('id'=>$fullbacklog['id']));
									$addlog=array(
										'uniacid'=>$_W['uniacid'],
										'openid'=>$order['openid'],
										'logid'=>$fullbacklog['id'],
										'status'=>1,
										'createtime'=>time()
										);
									pdo_insert('ewei_shop_fullback_recommend_log',$addlog);
								}
							}
						  }
						}
